...
interface BrazilJobsPageProps {
  searchParams: Promise<{ seniority?: string; remote?: string; q?: string }>
}

export default async function BrazilJobsPage({ searchParams }: BrazilJobsPageProps) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-7">
        <h1 className="text-[22px] font-bold text-[#EEEEFF]">Brazil Jobs</h1>
        <p className="text-[#6B6990] text-[13px] mt-0.5">
          {totalCount} open positions · Brazil · AI &amp; Data Engineering
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-7 flex-wrap">
        <div className="flex gap-1 bg-[#17153A] border border-white/[0.07] rounded-2xl p-1">
          {seniorityOptions.map(({ value, label }) => (
            <a
              key={value}
              href={`/jobs/brazil?seniority=${value}${remoteOnly ? '&remote=true' : ''}${query ? `&q=${query}` : ''}`}
        ...
        <a
          href={`/jobs/brazil?seniority=${seniority}&remote=${!remoteOnly}${query ? `&q=${query}` : ''}`}
        ...
        <form method="GET" action="/jobs/brazil" className="flex-1 min-w-48 max-w-sm">
          <input
            type="text"
            name="q"
            defaultValue={query}
